...
_(Le Sommaire sera généré automatiquement et finalisé une fois que le contenu intégral des chapitres aura été rédigé. Pour l'instant, il reprend le plan détaillé fourni.)_

---

## LISTE DES FIGURES ET TABLEAUX

_(À compléter au fur et à mesure de la rédaction, en s'assurant que toutes les illustrations (Diagrammes UML, Schémas d'Architecture, Captures d'écran, Tableaux de comparaison) sont numérotées et légendées.)_
...
_(Les annexes sont des pièces justificatives. Elles doivent être générées par vous-même à partir de votre code et de votre documentation.)_

### Annexe A : Extrait de Code Important (Sécurité JWT - Java)

_(Inclure l'extrait de code de la classe `JwtAuthenticationFilter.java` ou `SecurityConfig.java`)_

### Annexe B : Guide d'Installation

_(Détailler les étapes pour lancer l'application : Clonage du Repo Git, installation des dépendances (Maven/NPM), configuration de la base de données (fichier `application.properties`), commandes de lancement (e.g., `mvn spring-boot:run`, `ng serve`).)_

### Annexe C : Modèle Entité-Association Détaillé

_(Fournir le schéma complet de la base de données avec toutes les tables, colonnes et relations, souvent un document généré par l'outil SGBD ou un outil de modélisation.)_
